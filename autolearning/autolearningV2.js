...
const accounts = [
    { email: '052205012599', password: '052205012599' },
];
...
async function fetchUserInfo(session_id) {
    try {
        const res = await fetch('https://hoclythuyetlaixe.eco-tek.com.vn/mail/init_messaging', {
            method: 'POST',
            headers: {
                'accept': '*/*',
...
async function fetchLessons(slideUrl, session_id, subject) {
    try {
        ...
        const res = await fetch(`https://hoclythuyetlaixe.eco-tek.com.vn${slideUrl}`, {
            method: 'GET',
            headers: {
                'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
                'Accept-Language': 'en-US,en;q=0.9',
        ...
        const lessonItems = $('li.o_wslides_slides_list_slide.o_wslides_js_list_item');
        ...
        lessonItems.each((_, el) => {
            ...
            if (
                ...
                lessons.push({ title, link: href, slide_id: slideId, subject, completion: completionValue });
            }
        });

        console.log(`📚 Danh sách bài học có tiến độ hoàn thành nhỏ hơn 100% từ ${slideUrl}:`, lessons);
        return lessons;
    } catch (error) {
...
async function fetchLessonAPIs(lessonUrl, session_id, page, browser) {
    try {
        ...
        const waitForCountdownEnd = new Promise((resolve) => {
            ...
            page.on('request', async (request) => {
                ...
                if (['xhr', 'fetch'].includes(resourceType) || url.includes('/api/') || url.includes('json')) {
                    ...
                    if (url === 'https://hoclythuyetlaixe.eco-tek.com.vn/slide/countdown-end/') {
        ...
        await page.goto(`https://hoclythuyetlaixe.eco-tek.com.vn/${lessonUrl}?fullscreen=0`, {
            waitUntil: 'networkidle2',
            timeout: 60000,
        });
